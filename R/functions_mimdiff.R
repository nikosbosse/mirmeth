getcountsdata <- function(origin, raw = T, oldinput = F, collapseReplicates = T, grouped = F, cleaned = F){

  if (origin == "gaurav" && raw == T){
    # path <- "../data/countsgaurav/counts/raw_counts/mirna/"
    path <- "../data/daten_gaurav_run2/counts/raw_counts/mirna/"
  } else if (origin == "gaurav" && raw == F){
    path <- "../data/daten_gaurav_run2/counts/normalized_counts/mirna/"
  } 
  if (oldinput == T){
    path <- "../data/daten_gaurav_run2/counts/raw_counts/mirna_but_old_input2/"
  }
  if (origin == "2018") {
    path <- "../data/data_Cortex_August_2018/counts/raw_counts/mirna/"
  }
  if (origin == "2019") {
    path <- "../data/data_CA_August_2019/counts/raw_counts/allncrna/"
    #path <- "../data/data_CA_August_2019/counts/raw_counts/mirna/"
  ...
  all.files <- list.files(path = path,pattern = ".txt")
  ...
  if(origin == "2019") {
    #filter out all those that arent mmu or scrambled spike-ins
    index <- grepl(pattern = "mmu-", rownames(counts)) | grepl(pattern = "Scr_m", rownames(counts))

    counts <- counts[index, ]
   
    colnames(counts) <- paste(c(rep(c("Y_input", "Y_IP"), each = 4), 
                                rep(c("Old_input", "Old_IP"), each = 4)), 
                              c(rep(1:4, times = 2), rep(1:4, times = 2)),
                              sep = "")
  }

  if(origin == "2018") {
    #filter out all those that arent mmu or scrambled spike-ins
    colnames(counts) <- paste(c(rep(c("Y_input", "Y_IP"), times = 2), 
                                rep(c("Old_input", "Old_IP"), times = 2)), 
                              c(rep(1:2, each = 2), rep(1:2, each = 2)),
                              sep = "")
  }  


  if (origin == "gaurav_old"){
    colnames(counts) <- paste(rep(c("input", "iped"), 4), rep(1:4, each = 2),
                              sep = "")
  } else if (origin == "gaurav" && collapseReplicates == T) {
    counts[,4] <- counts[,4] + counts[,5]
    counts[,6] <- counts[,6] + counts[,7]
    counts[,8] <- counts[,8] + counts[,9]
    counts <- counts[, -c(5,7,9)]
    colnames(counts) <- paste(rep(c("input", "iped"), 4), rep(1:4, each = 2),
                              sep = "")
  } else if (origin == "gaurav" && collapseReplicates == F) {
    colnames(counts) <- c("input1", "iped1", "input2", "iped2", "iped2.2",
                          "input3", "input3.2", "iped3", "iped3.2",
                          "input4", "iped4")
  }

  if (grouped == T) {
    inp <- grepl("input", x = colnames(counts))
    ip <- grepl("ip", x = colnames(counts))
    counts <- cbind(counts[,inp], counts[,ip])
  }

  # omit last 4 rows that are filled with some stuff from gaurav
  # clean out everything that is completely zero
  if (cleaned == T) {
    n <- nrow(counts)
    n4 <- n - 4
    counts <- counts[-(n4:n),]
